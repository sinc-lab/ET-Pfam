...
ET-Pfam is a novel approach based on ensembles of deep learning (DL) classifiers trained with transfer learning to predict functional families in the Pfam database. 
...
This repository provides instructions for using a reduced dataset (the *mini dataset*) first, as it allows quick testing and replication of the main results with significantly lower time and computational costs. 

## 2. Test ET-Pfam ensembles

To reproduce the ensemble evaluation presented in the manuscript using pre-trained DL base models, you must download the test set embeddings, the trained base models and the ensemble weights (per model and per family). All these files are available in this [folder](https://drive.google.com/drive/folders/1p54V_g4iy-XGjzi0C7LGQP9brb9QnL1k?usp=sharing). 

### 2.1 Download test embeddings

The test embeddings are available in the compressed file [`data/embeddings/mini_test.tar.gz`](https://drive.google.com/file/d/1B0T5C8XiVheYTw6IEFAf3LOFsupzPLVs/view?usp=drive_link). You must download it and place it in a local folder called `data/embeddings/` inside the local repository folder `ET-Pfam`. While inside the repository root, run the following commands:
...
The file [`models/mini.tar.gz`](https://drive.google.com/file/d/1SFOHaZy7NtXohyqdT3-LIvH5sC67WyMy/view?usp=drive_link) contains both the trained base models and the ensemble weights. To download the entire folder use:
...
Note that this process, with all strategies evaluations, may take approximately up to 10 hours to complete on a single GPU. The results will be saved to the folder `results/mini/`, in the file `ensemble_metrics.csv`. Also, this folder will contain subdirectories with the ensemble results for each individual strategy.
...
To obtain the ensemble weights for the pre-trained base models downloaded in the previous section, you need to fit the ensemble using the development (dev) partition. Before running the training, ensure that you have downloaded both the trained base models and the test embeddings, provided earlier. 

### 3.1 Download dev embeddings

The dev embeddings are available in a compressed file named [`data/embeddings/mini_dev.tar.gz`](https://drive.google.com/file/d/15pHIO7J2awgwkgHR5d4MhK2mt4JC1j54/view?usp=drive_link), located in the provided Drive folder. 
...
If you want to train your own DL base models, you first need to download the training embeddings. These are located in the file [`data/embeddings/mini_train.tar.gz`](https://drive.google.com/file/d/12-NZPjjeiAKqp4A8gQ9KfIpSzuBawdMp/view?usp=drive_link) inside the provided Drive folder. You can download it, move it to the `data/embeddings/` folder in your local repository and extract its contents. Otherwise, you can obtain them running the following commands from the repository root:
...
By default, running the training script will train the base models using the settings defined in `config/base.json`. If you want to change hyperparameters, you can edit this JSON file before training. To edit the file directly from the command line using `nano`, run:

```
nano config/base.json
```

Inside the json file, you can modify values like the window length (`window_len`), learning rate (`lr`) batch size (`batch_size`), and more. To save and exit nano, press `Ctrl + O` to write the file, then Enter, and `Ctrl + X` to exit. 

After saving your changes, run the training script as shown in the previous step. 
...
Before running any steps for the full dataset, edit the config/base.json file as shown in [section 4.3](#43-customize-the-training-configuration). In particular, set the `dataset` field to `full`, to indicate that you are working with the full Pfam data instead of the mini version.

### 5.1. Test ET-Pfam ensembles

First, download the required dataset files from the Drive folder located in `data/full/` and place them into `data/full/` in your local repository. Alternatively, you can download them directly using:

```
gdown --folder 18bD2q-iL2MZE2ddH_-jRp_VDZXoZG8QP --output data/
```

Next, you need to download the test embeddings from the Drive folder located in [`data/embeddings/full_test.tar.gz`](https://drive.google.com/file/d/1HFsSCg-z7NqKwdMfxgj8y1-Ux1X7zJnJ/view?usp=drive_link), the trained base models and the ensemble weights for the full dataset. From the repository root, run:
